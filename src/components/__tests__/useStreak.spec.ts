import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest'
import { mount, flushPromises } from '@vue/test-utils'
import ChallengeList from '../ChallengeList.vue'
import ChallengeCard from '../ChallengeCard.vue'
import { useStreak } from '../../composables/useStreak'

const API_BASE = 'https://dailyhabit.onrender.com'
const STREAK_URL = `${API_BASE}/api/v1/streak`
const CHALLENGES_URL = `${API_BASE}/api/v1/challenges`

const okJson = (data: unknown) =>
  Promise.resolve({ ok: true, json: () => Promise.resolve(data) } as Response)

describe('Streak-Integration', () => {
  const fetchMock = vi.fn()

  beforeEach(() => {
    fetchMock.mockReset()
    vi.stubGlobal('fetch', fetchMock)
    const { streak, completedToday } = useStreak()
    streak.value = 0
    completedToday.value = false
  })

  afterEach(() => {
    vi.unstubAllGlobals()
    vi.restoreAllMocks()
  })

  it('ChallengeList.vue ruft GET /api/v1/streak beim Mounten auf und zeigt den Wert in der Info-Karte', async () => {
    // 1. Mock für LoadChallenges (leeres Array), 2. Mock für Streak
    fetchMock
      .mockReturnValueOnce(okJson([]))
      .mockReturnValueOnce(okJson({ streak: 5, completedToday: true }))

    const wrapper = mount(ChallengeList)
    await flushPromises()

    expect(fetchMock).toHaveBeenCalledWith(STREAK_URL)
    expect(wrapper.text()).toContain('5 Tage') // Die Karte zeigt jetzt "{{ streak }} Tage"
  })

  it('ChallengeList.vue ruft GET /api/v1/streak nach einem erfolgreichen Toggle erneut auf', async () => {
    const challenge = { id: 1, title: 'Workout', category: 'Fitness', done: false }

    fetchMock
      .mockReturnValueOnce(okJson([challenge]))                           // onMounted loadChallenges
      .mockReturnValueOnce(okJson({ streak: 0, completedToday: false }))  // onMounted refreshStreak
      .mockReturnValueOnce(okJson({}))                                    // PATCH /1/toggle
      .mockReturnValueOnce(okJson({ streak: 1, completedToday: true }))   // refreshStreak nach Toggle

    const wrapper = mount(ChallengeList)
    await flushPromises()

    wrapper.findComponent(ChallengeCard).vm.$emit('toggle', challenge)
    await flushPromises()

    expect(fetchMock).toHaveBeenNthCalledWith(3, `${CHALLENGES_URL}/1/toggle`, { method: 'PATCH' })
    expect(fetchMock).toHaveBeenNthCalledWith(4, STREAK_URL)
  })
})
